...
#with open(flash+'_%s_data_filt.bin'%cs[chp-1], # raw/filt
#          "wb") as file:
...
div=16 # division
#local_extrema(t1, t2, fsc, fsxe, tc, fcur, txes, fpvs, fefs, div)
...
# setup PLOTS
fig, (ax1, ax0, ax2) = plt.subplots(3, 1) #
...
#for ax in (ax1, ax0, ax2): ax.set_xlim(st, et)

# zoom
for ax in (ax1, ax0, ax2): ax.set_xlim(t1, t2) #
ax1.set_ylim(0, 4e7)
...
# frames relative to peak
frp = -1*np.array([322,321,320,319,318,317,  # start UNL
                   108,107,106,105,104,103,  # RL branch creat.
                     5,  4,  3,  2,  1,  0])-1 # RL - MP

for f in frp:
    et = 972.4675 + f/24 # event time (ms) # peak - frames before
    ...
    for ax in (ax1, ax0, ax2):
        ax.axvspan(cof-htw, cof+htw, color='red', alpha=1/3, zorder=0)
#ax0.text(968.575, 1., 'F888', fontsize='small')

#plt.show()

#fig.savefig(flash+'_Lie_whole_raw.pdf', bbox_inches='tight')
#fig.savefig(flash+'_Lie_whole_100khzI.pdf', bbox_inches='tight')
#fig.savefig(flash+'_Lie_zoomMP-F88_raw.pdf', bbox_inches='tight')
fig.savefig(flash+'_Lie_EFzoom_100khzI.pdf', bbox_inches='tight')
