# Author: Toma OC
#
# Generates subplots comparing I, dI/dt, E and XRE as functions of time
...
## CHANNEL 1 : X-rays
KK_xr = 0.4 # Volts into MeV

fIDx = open('adlink_ch0.bin', 'rb')#, buffering=0)
Ych0_r = np.fromfile(fIDx, dtype='>d') #, XX
#print(Ych0_r[0])
#print(type(Ych0_r[0]))
Ych0x = Ych0_r * KK_xr
...
# align current/derivative and x-ray/E-field data
...
print('X-ray avg.: %.7g +/- %.7g' % (np.mean(fxrs), np.std(fxrs)))
print('X-ray range: %.7g (at %.9g ms) to %.7g (at %.9g ms)'
      % (np.min(fxrs), txes[np.argmin(fxrs)],
         np.max(fxrs), txes[np.argmax(fxrs)]))
...
#with open(g.names[f]+'_EF_XR_data_raw.bin', "wb") as file: # raw/filt
#    np.save(file, [txes, fefs, fxrs])
...
print('E-field range: %.7g (at %.9g us) to %.7g (at %.9g us)'
      ...
      #'deltaE =',(Emax-Emin))
print('XR avg.: %.5g +/- %.5g'%(np.mean(fxrs[ir1:ir2]),np.std(fxrs[ir1:ir2])))
print('X-ray range: %.7g (at %.9g ms) to %.7g (at %.9g ms)'
      % (Xmin, txes[ir1+np.argmin(fxrs[ir1:ir2])],
         Xmax, txes[ir1+np.argmax(fxrs[ir1:ir2])]))

# Find and print local extrema
div=16 # division
#local_extrema(t1, t2, fsc, fsxe, tc, fcur, fbds, txes, fxrs, fefs, div)
...
Xmax,Xmin = (Xmax,Xmin)-np.mean(fxrs)

for y in (fcur, fbds, fefs, fxrs): y -= np.mean(y) #
...
ax1.plot(txes, fxrs, linewidth=0.8, color=colors[1])
...
for ax in (ax0, ax1, ax2, ax3): #
    ...
    # X-ray event times:
    #ax.axvline(x=959.0564, linestyle=':', lw=0.5, color=colors[5])
    #ax.axvline(x=959.2776, linestyle=':', lw=0.5, color=colors[5])
    #ax.axvline(x=960.409, linestyle=':', lw=0.5, color=colors[5])
    #ax.axvline(x=960.711, linestyle=':', lw=0.5, color=colors[5])
    #ax.axvline(x=960.376, linestyle=':', lw=0.5, color=colors[5])
    #ax.axvline(x=960.464, linestyle=':', lw=0.5, color=colors[5])
    #ax.axvline(x=960.6844, linestyle=':', lw=0.5, color=colors[5])
...
# X-ray event text
#ax1.text(902.599, -600, '$t_{\mathrm{XR3}}=902.5976$', fontsize='small')


# shade HSC frame region
# UP2 correction: -0.00625
# UN4 correction: +0.01349
#et = 972467.5 - 2/.024 # event time (us)
#cpt = tc[np.argmax(np.abs(fcur))] - 6.25 # I peak time - correction
#ef = round((et-cpt)*.024) # event frame (relative to peak)
#print('Event Frame:', ef)
#cof = cpt + ef/.024 # center of event frame
#print('Temporal Center of Event Frame (us):', cof)
#htw = .5/.024 # half temporal width
#for ax in (ax0, ax2, ax3): #ax1,
#    ax.axvspan(cof-htw, cof+htw, color='red', alpha=1/3)
#ax0.text(972360, 1., 'F974', fontsize='small')

#et = 959.2776 # event time (ms)
#ax0.text(959.273, 0.4, 'Frame 661', fontsize='small')
...
#  8. Scatter plot of rise-times, peaks, etc. (current, x-ray). DONE.
